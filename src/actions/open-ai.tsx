...
export async function generatePrompts(prompt: string) {
    const completion = await openai.beta.chat.completions.parse({
        model: "gpt-4o-mini",
        messages: [
            { role: 'system', content: 'You are an engine that makes book recommendations based on the books the user has read. You will return an array of json objects with the books name, author and isbn. please include at least 20 books' },
            {
                role: "user",
                content: prompt,
            },
